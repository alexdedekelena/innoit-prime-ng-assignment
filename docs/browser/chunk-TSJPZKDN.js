    ...
    if (isFunction(observerOrNext) || !observerOrNext) {
      ...
      if (this && config.useDeprecatedNextContext) {
        ...
        partialObserver = {
          ...
          complete:
            observerOrNext.complete && bind(observerOrNext.complete, context2),
        };
      } else {
...
function createInvalidObservableTypeError(input2) {
  return new TypeError(
    `You provided ${input2 !== null && typeof input2 === "object" ? "an invalid object" : `'${input2}'`} where a stream was expected. You can provide an Observable, Promise, ReadableStream, Array, AsyncIterable, or Iterable.`
  );
}
...
var RetrievingInjector = class {
  ...
  retrieve(token, options) {
    ...
    return this.injector.get(
      token,
      ngOptions.optional ? NOT_FOUND : THROW_IF_NOT_FOUND,
      ngOptions
    );
  }
};
  ...
  execute() {
    ...
    for (const phase of AFTER_RENDER_PHASES) {
      for (const sequence2 of this.sequences) {
        ...
        try {
          sequence2.pipelinedValue = this.ngZone.runOutsideAngular(() =>
            this.maybeTrace(() => {
...
var AfterRenderSequence = class {
  ...
  destroy() {
    ...
    if (scheduled2) {
      this.view[AFTER_RENDER_SEQUENCES_TO_ADD] = scheduled2.filter(
        (s) => s !== this
      );
    }
  }
...
function afterRenderImpl(callbackOrSpec, injector, options, once) {
  ...
  const sequence2 = new AfterRenderSequence(
    manager.impl,
...
function locateHostElement(
  ...
  const preserveContent =
    preserveHostContent || encapsulation === ViewEncapsulation.ShadowDom;
...
var REF_EXTRACTOR_REGEXP = /* @__PURE__ */ new RegExp(
...
function stringifyNavigationInstructions(instructions) {
  ...
  for (let i = 0; i < instructions.length; i += 2) {
    ...
    for (let r = 0; r < repeat; r++) {
      container.push(
        step === NODE_NAVIGATION_STEP_FIRST_CHILD ? "firstChild" : "nextSibling"
      );
    }
  }
...
function resolveHostDirectives(matches) {
  ...
  if (allDirectiveDefs !== null) {
    allDirectiveDefs.push(
      ...(componentDef === null ? matches : matches.slice(1))
    );
...
function resolveHostDirectivesForDef(
  ...
  ngDevMode &&
    assertDefined(
      ...
      "Expected host directive resolve function"
    );
  ...
  componentResourceResolutionQueue.forEach((component, type) => {
    ...
    if (component.styleUrl && component.styleUrls?.length) {
      ...
      component.styleUrls.forEach((styleUrl, index) => {
        styles.push("");
        promises.push(
          cachedResourceResolve(styleUrl).then((style2) => {
      ...
      promises.push(
        cachedResourceResolve(component.styleUrl).then((style2) => {
          styles.push(style2);
      ...
      if (typeof configOrFn === "function") {
        ...
        for (const config2 of resolved) {
          trackHostDirectiveDef(
            createHostDirectiveDef(config2),
            matchedDefs,
            hostDirectiveDefs
          );
        }
...
var DEFER_BLOCK_DEPENDENCY_INTERCEPTOR = /* @__PURE__ */ new InjectionToken(
...
function findDeferBlocks(node, lView, results) {
  ...
  for (const details of blocks) {
    ...
    const data = {
      ...
      incrementalHydrationState: inferHydrationState(
        tDetails,
        lDetails,
        registry
      ),
...
function shouldAttachRegularTrigger(lView, tNode) {
  ...
  const hasHydrateTriggers =
    tDetails.flags !== null && (tDetails.flags & 1) === 1;
  if (false) {
...
function transitiveScopesFor(type) {
  ...
  throw new Error(
    `${type.name} does not have a module def (\u0275mod property)`
  );
}
...
function bootstrap(config2) {
  const envInjector = isApplicationBootstrapConfig(config2)
    ? config2.r3Injector
    : config2.moduleRef.injector;
  ...
  return ngZone.run(() => {
    ...
    if (typeof ngDevMode === "undefined" || ngDevMode) {
      if (exceptionHandler === null) {
        const errorMessage = isApplicationBootstrapConfig(config2)
    ...
    if (isApplicationBootstrapConfig(config2)) {
      ...
      const onPlatformDestroyListeners = config2.platformInjector.get(
...
var ResourceImpl = class extends BaseWritableResource {
  ...
  set(value) {
    ...
    if (
      state2.status === ResourceStatus.Local &&
...
function projectStatusOfState(state2) {
  switch (state2.status) {
    case ResourceStatus.Loading:
      return state2.extRequest.reload === 0
        ? ResourceStatus.Loading
        : ResourceStatus.Reloading;
    case ResourceStatus.Resolved:
      return isResolved(untracked2(state2.stream))
...
@angular/animations/fesm2022/private_export-CacKMzxJ.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-TSJPZKDN.js.map
